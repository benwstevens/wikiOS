...
Three questions, in the model's own voice, close to these words:
- "There are already rules about how I should behave with you, baked into `LLM-rules.md`. You can read them, and we can edit them or I can suggest changes as we go. Is there anything I should keep in mind for now?" *(Record any answer as the first entry in `customization/LLM-rules-origins.md`; if it should govern every session, propose the one-line version for `LLM-rules.md` and mark the module or line switched off or on in `customization.md`.)*
- "The author had some customizations of his own: a file of negotiation rules, a guide to his writing style, his task list and meeting list. Do you already have anything like that, or want to set one up?" *(Anything they have is a key file, step 6. Anything they want becomes an open item in the hub. If they want the writing guide, start `customization/writing-style.md` from the template and explain the capture loop: after they send something you drafted, keep their final and the lesson.)*
- "A big part of growing the wiki will be my notes from your meetings. Do you type those out, or use a transcript service? Anything I should know about how they arrive?" *(The answer goes into a how-to under `customization/howtos/`, including any quirk such as a recorder that swaps speaker labels, and is what "update the wikis" reads as its main inflow.)*

### 9. The upkeep, explained once, then confirm
Tell the user, in these words or close to them: *"Here's how we stay up to date. I'll make a log of each conversation we have. You'll have meeting transcripts or notes that you save somewhere; tell me where, for each project. You may also make separate notes in a project's `log.md`. All of this is good information for the wikis. I'll ask you about adding basic information during each conversation or after each meeting. But once a week or so, you'll want me to do a scan across the whole system. That does two things: first, it makes sure everything has been incorporated, and if something is an important decision or viewpoint, I'll confirm it's actually your view before adding it. Then, after I add it, I check the wikis against each other to make sure there aren't any conflicts. The way you kick this off is by saying **'update the wikis'**. Anything you want to change about that?"* Record where meeting notes are saved (per project or one place) in `customization.md`. Then read `customization.md` back in full, take corrections, and build.
