...
An LLM-maintained wiki system for keeping durable, auditable knowledge in plain text files you own. Point your LLM at this folder and it sets itself up by asking you questions.
...
1. Memory lives in files, not in the model. Everything is text you can open, check, and fix, and the files win over what the model thinks it remembers.
2. One home per fact; link, never restate. A short always-on hub points to subject pages that are read only when needed.
3. Every claim names its dated source, and every page is copied before it is changed. Unsourced means flagged, not asserted.
4. Facts and notes are written directly; positions, contradictions, and anything on a hub page are proposed and wait for your yes.
5. Decisions are proposed for the wiki as they happen; every session leaves a log; meeting notes catch the rest. One command, "update the wikis," sweeps it all with your approval and ends with a check for contradictions. Nothing runs on a timer.
...
**Kept from the gist:** the three layers (your raw files, a wiki the model writes, a schema that disciplines it), the three operations (ingest, query, lint), an index the model reads first, an append-only log, and the division of labour where you curate and the model does the bookkeeping. **Added in wikiOS:** a dated source on every claim; a hard line between facts (written directly) and positions (asked about first); a copy of every page before it changes; session logs and meeting notes as the main inflow; folders with privacy walls between them; fixed page shapes for matters and subjects; rules for how the model behaves in conversation, not just in the wiki; one owner-triggered refresh that ends with a lint instead of a schedule; the rule that the wiki is not an inventory of your files; versioned rule files that several installs can share; and the setup interview itself. Karpathy's gist: https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f
...
- Daily work never touches this repo: the freshness check (wiki-os §1) runs entirely inside the install, offline.
